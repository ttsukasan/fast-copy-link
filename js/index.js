...
const fileUrls = [
  ...
  "js/vendor/copyLinkPlaintext.js"].map(path => `${window.location.pathname}${path}`);
Promise.all(fileUrls.map(url =>
  ...
  const button1 = document.getElementById('scriptMarkdown');
  button1.href = `javascript:${result1.code}void(0);`;
  button1.classList.remove('hidden');

  const result2 = await minify(fileContents[2], {sourceMap: true});
  const button2 = document.getElementById('scriptPlaintext');
